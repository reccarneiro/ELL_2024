...
# plt.show()
plt.savefig(os.path.join('Figures', 'inflation_mom.png'))
plt.close()
...
for seed in seed_list:
    ...
    Validation_Err['RA',seed] = pd.DataFrame()
    Validation_Err['RA',seed]['average_n_list'] = average_n_list
    Validation_Err['RA',seed]['val_err'] = val_err_RA
    ...
    n_estimators_list = np.arange(1,100,3)
    val_err = np.zeros((n_val,len(n_estimators_list)))
    XGBmodel_dict = {}
    for cv_i, n_estimators in enumerate(n_estimators_list):
    ...
    K_test = get_Gram_rbf(X_train_stzd.values, X_val_stzd.values, n_train, n_test, gamma)
    ...
    out = np.concatenate((RW_out.values, AR1_out.values, AR12_out.values, AR1_12_out.values, AR_out.values, RA_out.values, RF_out.values, XGB_out.values, XGBs_out.values, Lasso_out.values, Ridge_out.values, AdaLasso_out.values, PCR_out.values, RKHS_out.values, NN_out.values), axis=0)
...
file_name ='Validation_Err_MoM_%s.pkl'%(Transformation.replace(' ','_'))
